...
export default function Overview({tutors, subject, accessToken}: PageProps) {
    ...
    const redirectToTutorPage = (id: string, accessToken: string) => {
        router.push({
                pathname: `http://localhost:3000/tutor/${id}`,
                        ...
                        {tutors?.map(tutor => {
                                return (
                                    <Card
                                        ...
                                        borderRadius='20'
                                        data-cy="card"
                                        onClick={() => redirectToTutorPage(tutor._id, accessToken)}>
                                        <Box
                                            height="40%">
                                            <Image
                                                objectFit='cover'
    ...
    export const getServerSideProps: GetServerSideProps = async (context) => {
        const accessToken = JSON.stringify(context.req.cookies.access_token != undefined ? context.req.cookies.access_token : null)
